...
class AuthService extends AbstractAuth{
	...
	public function getIdentifier(): ?int{ return !is_null($this->user) ? $this->user->getIdentifier() : null;}
	public function whoAmI(): ?UserInterface{ return !is_null($this->user) ? User::pick($this->user->getIdentifier()) : null; }
	protected function find($login): ?UserInterface{ return User::search(Filter::where(User::email()->is($login)))->pick(); }
